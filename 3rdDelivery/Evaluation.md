...
3) Did you know that originally the statues were coloured and not white as they are now?
4) Would you like to see a reconstruction of the statues as they were original? (so with colours and all the missing parts)?
5) Do you know what a hologram is?
6) Do you think you'd like to visit a museum more if, in addition to seeing the ancient statues, you could see a hologram that represents the statues in their original shape and colours?
...
4) yes
5) Yes
6) I think it would enrich the visit
...
We chose the Think aloud method that is a user-based evaluation method. In this technique, every user is encouraged to do a specific task and to comment on what he/she is doing and why.
The experiment consisted in making each user try all three possibilities.

Type 1: interaction with objects
In this case, we couldn't use real objects with NFC code, so we simulated them by making the user choose an object and sending the signal using the phone.
...
The aesthetic effect of the hologram: average grade 9/10
...
Users must have the best possible experience when using the system.
